...
The **Laplace approximation** is used to approximate integrals of functions that are $\mathrm{O}(n)$ of the form 
\begin{align*}
I_n = \int_{\mathbb{R}_p} g(\boldsymbol{x}) \mathrm{d} \boldsymbol{x} = \int \exp\{h(\boldsymbol{x})\}\mathrm{d} \boldsymbol{x}.
\end{align*}

The idea is that we can, ignoring terms above third order and assuming  $\boldsymbol{x}_0$ satisfies $h'(\boldsymbol{x}_0)=\boldsymbol{0}_p,$
...
\begin{align*}
 I_n \approx \left(\frac{2\pi}{n}\right)^{p/2} | \mathbf{H}(\boldsymbol{x}_0)|^{-1/2}\exp\{h(\boldsymbol{x}_0)\} + \mathrm{O}(n^{-1})
\end{align*}
...
## Prior and likelihood contribution
...
\begin{align*}
 \log p(\boldsymbol{y}) &= \log p(\boldsymbol{y} \mid \widehat{\boldsymbol{\theta}}_{\mathrm{MLE}}) -\frac{p}{2} \log n + \\& \quad  \log p(\widehat{\boldsymbol{\theta}}_{\mathrm{MLE}}) - \frac{1}{2} \log |\boldsymbol{\imath}(\widehat{\boldsymbol{\theta}}_{\mathrm{MLE}})| + \frac{p}{2} \log(2\pi)  + \mathrm{O}(n^{-1/2})
\end{align*}
where the error is now $\mathrm{O}(n^{-1/2})$ due to replacing the true information by it's sample counterpart. 

## Approximation error

The likelihood is $\mathrm{O}(n),$ the second term of the first line is $\mathrm{O}(\log n)$ and the others are $\mathrm{O}(1).$ This approximation is thus overall $\mathrm{O}(1).$
...
then the approximation error to the marginal likelihood reduces to $\mathrm{O}(n^{-1/2}).$ 

## Frequentist approximation to marginal likelihood
...
The $\mathsf{BIC}$ approximation will improve with increasing sample size.
...
H <- -numDeriv::hessian(logpost, x = map)
# Laplace approximation
marg_lik_laplace <- 1/2*log(2*pi) - c(determinant(H)$modulus) + logpost(map)
```
...
\end{align*}
While the Laplace method has an error $\mathrm{O}(n^{-1}),$ the leading order term of the expansion cancels out from the ratio. 
...
Consider a model with response $\boldsymbol{y}$ which depends on covariates $\mathbf{x}$ through a latent Gaussian process, typically the prior of coefficients $\boldsymbol{\beta} \in \mathbb{R}^p.$
Often, $\boldsymbol{\beta}$ is Gaussian Markov random field with a sparse precision.


The dimension $p$ can be substantial (several thousands) with a comparably low-dimensional hyperparameter vector $\boldsymbol{\theta} \in \mathbb{R}^m.$
...
The second step is to exploit the local dependence on $\boldsymbol{\beta}$ using the Markov structure to build an improvement to the Hessian. Various strategies are proposed in @Rue.Martino.Chopin:2009 and @Wood:2019. Nowadays, the INLA software uses a low-rank variational correction to Laplace method, proposed in @vanNiekerk.Rue:2024.
