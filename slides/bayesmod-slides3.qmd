...
- invariant to reparametrization (Jeffrey, penalized complexity)
- shrinkage priors (variable selection, reduce overfitting)

## Multivariate parameters

In multiparameters models, we often take the parameters to be (conditionally) independent a priori.
...
- These priors are not invariant: if $p\{\log(\sigma)\} \propto 1$ implies $p(\sigma) \propto \sigma^{-1}.$
...
If $Y \sim \mathsf{Poisson}(\mu)$ with density $f(y) = \mu^x\exp(-\mu)/x!,$ then for $\mu \sim \mathsf{gamma}(\alpha, \beta)$ with $\alpha, \beta$ fixed. Consider an i.i.d. sample with mean $\overline{y}.$ The posterior density is

$$
p(\mu \mid y) \stackrel{\mu}{\propto} \mu^{n\overline{y}} \exp\left(-\mu n\right) \mu^{\alpha-1} \exp(-\beta \mu)
$$
so must be $\mathsf{gamma}(n\overline{y} + \alpha, n + \beta).$
...
- trial-and-error using the prior predictive
   ...
   - for each, generate a new observation from the model $f(y_{\text{new}} \mid \boldsymbol{\theta}_0)$ via the prior prediction distribution.
...
If $p(\boldsymbol{\theta})$ is a proper prior and the likelihood $\mathcal{L}(\boldsymbol{\theta}; \boldsymbol{y})$ is well-defined, then the posterior $$p(\boldsymbol{\theta} \mid \boldsymbol{y}) \stackrel{\boldsymbol{\theta}}{\propto} \mathcal{L}(\boldsymbol{\theta}; \boldsymbol{y})p(\boldsymbol{\theta})$$ is almost surely proper.

:::

- If we pick an improper prior, however, we need to check that the posterior is well-defined.
...
Consider a Gaussian one-way ANOVA (random effect) model with $n$ independent observations in $J$ groups.

The $i_{(j)}$th observation in group $j$ is
\begin{align*}
Y_{i_{(j)}j} &\sim \mathsf{Gauss}(\mu + \alpha_j, \sigma^2) \\
\alpha_j &\sim \mathsf{Gauss}(0, \tau^2)
\end{align*}

The parameters of the model are $\mu, \sigma^2, \tau^2$, and the population parameters $\alpha_1, \ldots, \alpha_J.$
...
<!--
## An example of truncating priors
...
It needs not be a proper distribution.

## Proof 

Consider a bijective transformation $\theta \mapsto \vartheta.$ Under suitable regularity conditions, the chain rule implies that 
\begin{align*}
...
\end{align*}
Since the score has mean zero, $\mathsf{E}\left\{\partial \ell(\theta)/\partial \theta\right\}=0$ and so
\begin{align*}
...
Since $\mathsf{E}(Y)=\theta,$ the Fisher information is $$\imath(\theta) = \mathsf{E}\{\jmath(\theta)\}=1/\theta + 1/(1-\theta) = 1/\{\theta(1-\theta)\}.$$
...
## Power-scaling of prior

Rerunning the analysis with different priors can be expensive.

An alternative idea considers power scaling of the prior distribution, 
$$
p(\boldsymbol{\theta})^{\alpha}p(\boldsymbol{y} \mid \boldsymbol{\theta}), \qquad \alpha > 0;
$$
the case $\alpha=1$ corresponding to the model.

## Power-scaling with simulated samples

Suppose that we are generating posterior samples with $B$ simulations $\boldsymbol{\theta}_1, \dots \boldsymbol{\theta}_B.$

If we keep track of both

- the prior $p(\boldsymbol{\theta}_b)$ and 
- the likelihood $p(\boldsymbol{y} \mid \boldsymbol{\theta}_b)\propto \mathcal{L}(\boldsymbol{\theta}_b)$ 

for each posterior draw $\boldsymbol{\theta}_b,$ we can use importance sampling [@Kallioinen:2023] to estimate the posterior for different values of $\alpha$ without re-estimating the model.

This is implemented in the **R** package [`priorsense`](https://n-kall.github.io/priorsense/index.html) [@Kallioinen:2026].
...
* Priors are chosen before viewing the data.
* From an asymptotic perspective, the prior contribution is constant while the contribution of the likelihood typically grows linearly with the sample size.
* This is not necessarily the case for parameters that are poorly resolved (typically in hierarchy).

## Summary (2/3)

* Some priors are chosen for computational convenience
  - conjugate priors lead to closed-form (only exponential family)
  - improper, vague or flat priors
* Improper priors may lead to improper posterior.
* Priors are generally not invariant to reparametrization.
   - penalized-complexity or Jeffrey priors (1D only) are, by construction.
   


## Summary (3/3)

* Reasonable parameter values can be specified 
   - through moment matching
   - after looking at prior predictive simulations.
* Conducting prior sensitivity analyses (via power scaling or otherwise) is necessary to understand the impact of the prior.
