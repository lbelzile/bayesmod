...
The beta distribution with shapes $\alpha>0$ and $\beta>0$, denoted $\mathsf{beta}(\alpha,\beta)$, has density
...
## Illustration of beta distributions
...
g1 <- ggplot() +
  ...
  labs(x = "", y = "density", caption = "beta(0.5, 1)") +
  theme_classic()
g2 <- ggplot() +
    stat_function(fun =  function(x){
        dbeta(x, shape1 = 1.5, shape2 = 1)},
        alpha = 0.2,
        col = "#17154f",
        fill = "#17154f",
        geom = "area") +
  ...
  scale_x_continuous(
    ...
    breaks = seq(0,1, by = 0.25), 
    labels = c("0","0.25","0.5","0.75","1")) + 
  labs(x = "", y = "density", caption = "beta(1.5, 1)") +
  theme_classic()
g3 <- ggplot() +
    stat_function(fun =  function(x){
        dbeta(x, shape1 = 0.5, shape2 = 1.5)},
        alpha = 0.2,
        col = "#17154f", 
        fill = "#17154f",
        geom = "area") +
  ...
  scale_x_continuous(
    ...
    breaks = seq(0,1, by = 0.25), 
    labels = c("0","0.25","0.5","0.75","1")) + 
  labs(x = "", y = "density", caption = "beta(0.5, 1.5)") +
  theme_classic()
g4 <- ggplot() +
    stat_function(fun =  function(x){
  ...
  scale_x_continuous(
    ...
    breaks = seq(0,1, by = 0.25), 
    labels = c("0","0.25","0.5","0.75","1")) + 
  labs(x = "", y = "density", caption = "beta(0.5, 0.5)") +
  theme_classic()
g5 <- ggplot() +
    stat_function(fun =  function(x){
  ...
  scale_x_continuous(
    ...
    breaks = seq(0,1, by = 0.25), 
    labels = c("0","0.25","0.5","0.75","1")) + 
  labs(x = "", y = "density", caption = "beta(1, 1)") +
  theme_classic()
g6 <- ggplot() +
    stat_function(fun =  function(x){
  ...
  scale_x_continuous(
    ...
    breaks = seq(0,1, by = 0.25), 
    labels = c("0","0.25","0.5","0.75","1")) + 
  labs(x = "", y = "density", caption = "beta(1.5, 1.5)") +
  theme_classic()
(g1 + g2 + g3) / (g4 + g5 + g6)
```
...
We write $Y \mid \theta \sim \mathsf{binom}(n, \theta)$ for $\theta \in [0,1]$; the likelihood is $$L(\theta; y) = \binom{n}{y} \theta^y(1-\theta)^{n-y}.$$
...
## Marginal likelihood of beta-binomial model
...
but accurate approximations are still elusive [@Methodiev:2025].

In more complicated models, we will try to sample from the posterior directly by bypassing completely the calculation of $p(\boldsymbol{y}).$
...
- For the posterior density to be *proper*,
   - the marginal likelihood must be finite!
   - We need $\int p(\boldsymbol{y} \mid \boldsymbol{\theta})p(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta} < \infty$ for any datum $\boldsymbol{y}.$ 
   - It is therefore desirable to have a proper prior $p(\boldsymbol{\theta})$ satisfying $\int p(\boldsymbol{\theta})\mathrm{d} \boldsymbol{\theta} < \infty.$
...
#| eval: true
#| echo: false
#| fig-cap: "Scaled binomial likelihood for six successes out of 14 trials, with $\\mathsf{beta}(3/2, 3/2)$ (left), $\\mathsf{beta}(1/4, 1/4)$ (middle) and $\\mathsf{unif}[0,1/2]$ (right) priors and posterior density."
library(ggplot2)
library(MetBrewer)
library(patchwork)
...
We can compute moments of the posterior as summary statistics:

$$
\int_{\boldsymbol{\Theta}}g(\boldsymbol{\theta}) p(\boldsymbol{\theta} \mid \boldsymbol{y}) \mathrm{d} \boldsymbol{\theta} =  \int_{\boldsymbol{\Theta}}g(\boldsymbol{\theta}) \frac{p(\boldsymbol{y} \mid \boldsymbol{\theta})p(\boldsymbol{\theta})}{p(\boldsymbol{y})} \mathrm{d} \boldsymbol{\theta}
$$
but analytical approximations would require knowledge of the marginal likelihood $p(\boldsymbol{y}).$
...
The prior predictive is useful for determining whether the prior is informative or sensical

## Analytical derivation of predictive distribution

Given the $\mathsf{beta}(a, b)$ prior or posterior, the predictive for $n_{\text{new}}$ trials is beta-binomial with density
\begin{align*}
...
## Computations of credible intervals
...
* Bayesians treat both parameter and observations as random, the former due to uncertainty about the true value.
    - Inference is performed conditional on observed data, and summarized in the posterior.
* Bayesians specify both 
    - a prior for the parameter
    - a likelihood specifying the data generating mechanism. 
* Sequential updating (new likelihood contributions) leads to update of the posterior distribution.

## Summary (2/3)

* Bayesian inference is complicated by the fact that there is more often than not no closed-form expression for the posterior distribution. 
     - Evaluation of the normalizing constant, the so-called marginal likelihood, is challenging, especially in high dimensional settings.
     - Bayesians typically use approximations to the posterior distribution, or draw samples from it.
...
* The posterior predictive distribution is used for model assessment and prediction
   - it has a higher variance than the data generating distribution due to parameter uncertainty.
* Loss functions can be used to summarize a posterior distribution into a numerical summary of interest.
