...
- For the posterior density to be *proper*,
   ...
   - we need $\int p(\boldsymbol{y} \mid \boldsymbol{\theta})p(\boldsymbol{\theta} \mathrm{d} \boldsymbol{\theta} < \infty$ for any datum $\boldsymbol{y}.$ It is therefore desirable to have a proper prior $p(\boldsymbol{\theta})$ satisfying $\intp(\boldsymbol{\theta})\mathrm{d} \boldsymbol{\theta} < \infty.$
