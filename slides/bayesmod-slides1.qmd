...
\begin{align*}
L(\theta; y)p(\theta) & \stackrel{\theta}{\propto} p^{y}(1-p)^{n-y} \times p^{\alpha-1} (1-p)^{\beta-1}
...
A loss function $c(\boldsymbol{\theta}, \boldsymbol{\upsilon})$ is a mapping from $\boldsymbol{\Theta} \to \mathbb{R}^k$ that assigns a weight to each value $\boldsymbol{\theta}$, corresponding to the regret or loss arising from choosing this value. 


The corresponding point estimator $\widehat{\boldsymbol{\upsilon}}$ is the minimizer of the expected loss
...
The posterior mode $\boldsymbol{\theta} = \mathrm{argmax}_{\boldsymbol{\theta}} p(\boldsymbol{\theta} \mid \boldsymbol{y})$ is the **maximum a posteriori** or MAP estimator.
...
g1 <- ggplot() +
  ...
  geom_vline(xintercept = 3/4,
             colour = cols[2]) + 
  geom_label(mapping = aes(x = 4, y = 0.9*ymax[1], label = "mode"), 
             fill = "white", colour = cols[1],
             label.r = unit(0, "lines")) +
  geom_label(mapping = aes(x = 4, y = 0.75*ymax[1], label = "mean"),
             fill = "white", colour = cols[2],
             label.r = unit(0, "lines")) +
  geom_label(mapping = aes(x = 4, y = 0.6*ymax[1], label = "median"),
