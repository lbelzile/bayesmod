...
# Exercise 1.1 - **for credit**

Consider the geometric distribution with mass function
\begin{align*}
f(y; p) = p(1-p)^{y}, \qquad y = 0, 1, 2, \ldots;
\end{align*}
the latter is used to model the number of failures $Y$ from independent trials until a first success, the latter with probability $p$.
...
d. Use forward sampling (drawing first observations from $P$, then from $Y \mid P$ to simulate observations from the marginal of $Y$. Compare the distribution with that of $Y \mid p$ using a bar plot and verify the formulas for the expected value and variance derived previously using Monte Carlo integration.
