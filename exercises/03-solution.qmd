...
**Difficult**: Consider a simple random sample of size $n$ from the inverse Gaussian (Wald) distribution, with density
\begin{align*}
f(y; \nu, \lambda) = \left(\frac{\lambda}{2\pi y^{3}}\right)^{1/2} \exp\left\{ - \frac{\lambda (y-\nu)^2}{2\nu^2y}\right\}\mathrm{I}(y > 0)
\end{align*}
for location $\nu >0$ and shape $\lambda>0$. 
...
c. Derive the marginal posterior $p(\lambda \mid \boldsymbol{y})$.



::: {.solution}

a. First, note that the prior is for $g(\nu) = 1/\nu$, so the change of variable to get the corresponding function for $\nu$ gives $p(1/\nu) / \nu^2$, including the Jacobian of the transformation.

The exact parametric form of the product of prior (conditional for $\nu \mid \lambda$ and that of $\lambda$ is 

\begin{align*}
p(\nu, \lambda)&= \frac{\beta^{\alpha}}{\Gamma(\alpha)}\lambda^{\alpha-1}\exp(-\beta \lambda) \frac{(\lambda\tau)^{1/2}}{\sqrt{2\pi}\nu^2}
\exp\left\{-\frac{\tau\lambda}{2} \left(\nu^{-1}-\mu\right)^2\right\}
\\&=\stackrel{\nu,\lambda}{\propto} \frac{\lambda^{\alpha - 1/2}}{\nu^2}\exp \left\{ -\frac{\lambda}{2} \left(\frac{\tau}{\nu^{2}} - 2\frac{\tau\mu}{\nu} + \tau\mu^2 + 2\beta\right)\right\}
\end{align*}

To show conjugacy, we must prove that the posterior is of the same family as the prior. 

The likelihood contribution for an independent and identically distributed sample is proportional to
$$
p(\boldsymbol{y} \mid \lambda, \nu) \stackrel{\lambda, \nu}{\propto} 
\lambda^{n/2} \exp\left\{ -\frac{\lambda}{2} \left( \frac{1}{\nu^2}\sum_{i=1}^n y_i - \frac{2n}{\nu} +\sum_{i=1}^n y_i^{-1}\right)\right\}.
$$

Write $t_1(\boldsymbol{y}=\sum_{i=1}^n y_i$ and $t_2(\boldsymbol{y}) = \sum_{i=1}^n y_i^{-1}$. Multiplying the likelihood with the joint prior and expanding the squares in the exponential terms, we get
\begin{align*}
...
b. We see that the joint posterior is the product of two densities, say $p(\lambda \mid \boldsymbol{y})p(1/\nu \mid \lambda)$; $p(\lambda \mid \boldsymbol{y})$ is the density of a gamma distribution with shape $a = (n+1)/2+\alpha$ and rate $b=\beta + (c_3-c_1c_2^2)/2$, whereas the other is a reciprocal Gaussian with mean parameter $c_2$ and precision $\lambda c_1$.

c. To get the marginal $p(\lambda \mid \boldsymbol{y})$, we need to integrate with respect to $\nu$. Making the change of variable $\xi = 1/\nu$, we recover a Gaussian kernel
\begin{align*}
p(\lambda \mid \boldsymbol{y}) &= \lambda^{a-1}\exp(-b\lambda)\int_{\mathbb{R}}\exp\left\{ -\frac{\lambda c_1}{2}\left(\xi - c_2\right)^2\right\}  \mathrm{d} \xi 
...
rootSolve::multiroot(f = beta_moments,
                     ...
                     mean = 0.1, 
                     variance = 0.04)
# Alternative 2 :  minimize an objective function
# l2 norm of the difference between parameters
beta_match <- function(par, mean, variance){
  # Make unconstrained
  alpha <- exp(par[1]); beta <- exp(par[2]) 
  sqrt((alpha/(alpha+beta) - mean)^2 +
        (alpha*beta/(alpha+beta)^2/(alpha+beta+1) - variance)^2)
}
...
exp(optim(
  ...
  mean = 0.1, 
  variance = 0.04)$par)
# Could do even simpler and isolate a parameter
# Leads to a univariate problem
...
and we find, regardless of the method we use, the same solution: $\alpha = 0.125$ and $\beta=1.125$.
