...
\begin{align*}
p(\boldsymbol{y}) = \frac{L(\lambda) p(\lambda)}{p(\lambda \mid \boldsymbol{y})}
\end{align*}
so we get 
\begin{align*}
L_1(\boldsymbol{y}) &= (n\overline{y}+\kappa)^{n+1}/n!,\\ 
L_2(\boldsymbol{y}) &= L_1(\boldsymbol{y}) \times \gamma\{n+1, (n\overline{y}+\kappa)\}, \\ L_3(\boldsymbol{y}) &= (n\overline{y} + \beta)^{n+\alpha}/\Gamma(n\overline{y} + \beta).
\end{align*}
...
As the sample size increase, the curve becomes more concentrated around the maximum a posteriori value, which approaches the maximum likelihood estimator regardless of the prior function (if it is untruncated).

The truncated distribution (bottom left) is clearly identified from the "stubborn" prior, so 4C. Prior 3 has an average of 7, both the remaining two have a mean of 5, so 3D. We can look at how pronounced the posterior are: the $\mathsf{Ga}(50,10)$ is much more informative and the posterior mean is closer to the prior mean, so 1A and so 2B.
...
The purpose of this exercise is to show that numerical integration won't get you very far.
...
(norm_const_1 <- 1/sum(unnorm_post(grid)))
(norm_const_2 <- 1/icst_lambda$value)
  
# Check that the normalized value
cst_test <- integrate(f = function(x){unnorm_post(x)*norm_const_2}, 
                      lower = 0,
                      upper = 100)
cst_test
norm_const_3 <- norm_const_2/cst_test$value

```

Note that, because the integral is nearly zero, there is significant risk of numerical underflow and the reciprocal of the integrals are one order of magnitude apart, even when the sample size is not large. It is better to build an approximation only at the mode and match the curvature of the posterior there (using, e.g., a quadratic approximation to the posterior density). The absolute error is of the same order as the reciprocal normalizing constant, even for a one-dimensional problem!


For the posterior density, we can simply use the approximation with $p(\lambda \mid \boldsymbol{y}) \approx \widehat{c} L(\lambda)p(\lambda)$. The posterior density for the reciprocal rate is $p(\lambda \mid \boldsymbol{y}) \approx \widehat{c} L(\omega^{-1})p(\omega^{-1})\omega^{-2}$ (don't forget the Jacobian)!
...
library(ggplot2)
ggplot() + 
  stat_function(fun = function(x){unnorm_post(1/x)*norm_const_3/x^2},
                xlim = c(0, 30),
                n = 1001) +
  labs(y = "", 
