...
\frac{\partial}{\partial \vartheta} (\alpha-1)\vartheta - (\beta-1)\log\{1+\exp(\vartheta)\}
\\&=(\alpha-1) - \mathrm{expit}(\vartheta)(\beta-1)
\end{align*}
so that $\widehat{\vartheta}_{\mathrm{MAP}}=\mathrm{logit}\{(\alpha-1)/(\beta-1),$ and 
\begin{align*}
\jmath_{\vartheta}(\vartheta) = -\frac{\partial^2 k(\vartheta)}{\partial \vartheta^2} &= (\beta-1)\frac{\mathrm{expit}(\vartheta)}{1+\exp(\vartheta)}
\end{align*}
and the Jacobian of the change of variables gives 
\begin{align*}
